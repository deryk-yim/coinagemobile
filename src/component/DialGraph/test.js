...
calculateArcCircle = (index0, segments, radius, startAngle0 = 0, angleLength0 = 2 * Math.PI) => {
    ...
    // adding 0.005 to start drawing a little bit earlier so segments stick together
...
export default class DialGraph extends Component {
    ...
    componentWillMount = () => {
        this._sleepPanResponder = PanResponder.create({
            ...
            onPanResponderMove: (evt, { moveX, moveY }) => {
                ...
                let newAngle = Mat.atan2(moveY - circleCenterY, moveX - circleCenterX) + Math.PI / 2;
        ...
        this._wakePanResponder = PanResponder.create({
            ...
            onPanResponderGrant: (evt, gestureState) => this.setCircleCenter(),
            onPanResponderMove: (evt, { moveX, moveY }) => {
                const { circleCetnerX, circleCenterY } = this.state;
                const { angleLength, startAngle, onUpdate } = this.props;
    ...
    render() {
        const { startAngle, angleLength, segments, strokeWidth, radius, gradientColorFrom, gradientColorFrom, graidentColorTo,
            bgCircleColor, showClockFace, clockFaceColor, startIcon, stopIcon } = this.props;
        ...
        return (
            <View style={{ width: containerWidth, height: containerWidth }} onLayout={this.onLayout}>
                    ...
                    <Defs>
                        {
                            range(segments).map(i => {
                                const { fromX, fromY, toX, toY } = caculateArcCircle(i, segments, radius, startAngle, angleLength);
                ...
                <G transform={{ translate: `${strokeWidth / 2 + radius + 1}, ${strokeWidth / 2 + 1}` }}>
                    <Circle
                    />
                </G>
            </View>
